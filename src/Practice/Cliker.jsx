...
function Cliker() {
  ...
  const [filteredQuotes, setFilteredQuotes] = useState(dummy);

  const SearchFun = (query) => {
    const filtered = dummy.filter((s) =>
      s.quote.toLowerCase().includes(query.toLowerCase())
    );
  ...
  return (
    <div>
      ...
      />
      <button
        style={{ margin: '20px', width: '140px', height: '40px' }}
        onClick={() => SearchFun(change)}
      >
        Search
      </button>
      <br />
      <br />
      <ul>
        {filteredQuotes.map((e) => (
          <li key={e.id} style={{ border: '1px solid green' }}>
            {e.quote}
          </li>
        ))}
      </ul>
